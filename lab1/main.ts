...
function triangle(firstValue: number, firstSide: string, secondValue: number, secondSide: string): string {
  let a: number, b: number, c: number, alpha: number, beta: number;

  const isValid: string[] = ["leg", "hypotenuse", "adjacent angle", "opposite angle", "angle"];
